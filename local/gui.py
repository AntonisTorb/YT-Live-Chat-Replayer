...
def data_selection_window(settings:dict[str, str|int|tuple[int,int]|bool]) -> dict[str, bool|Path|None]:
    ...
    layout = [
        ...
        [
            sg.Checkbox("Ignore users in file:", key="-IGNORE_USERS-", 
                        enable_events=True, background_color=settings["BG_COLOR"]), 
            sg.Input("", readonly=True, key="-IGNORE_USER_PATH-", expand_x=True), 
            sg.Button("Browse", key="-BROWSE_USER_IGNORE-")
        ],
    ...
    while True:
        ...
        match event:
            ...
            case "-BROWSE_USER_IGNORE-":
                users_to_ignore = sg.popup_get_file('', no_window= True, file_types=(("text file", "*.txt*"),))
                if users_to_ignore:
                    data_window["-IGNORE_USER_PATH-"].update(users_to_ignore)
                    data["ignore user path"] = Path(users_to_ignore)
            ...
            case "-IGNORE_USERS-":
                data["ignore users"] = True if values["-IGNORE_USERS-"] else False
            case "Start":
                if data["json path"] is None:
                    ...
                    user_messages.one_line_error_handler("Please specify the highlight username file.", settings)
                elif data["ignore users"] and data["ignore user path"] is None:
                    user_messages.one_line_error_handler("Please specify the ignore username file.", settings)
                else:
                    data["exit"] = False
                    break
    data_window.close()
...
def display_chat(chat_window:sg.Multiline, 
    ...
    for comment in comment_deque:
        ...
        chat_window.update(f"{comment[1]}:", append=True, 
...
def main_window() -> None:
    ...
    if data["ignore users"]:
        with open(data["ignore user path"], "r") as file:
            usernames_to_ignore = [user[:-1] if user.endswith("\n") else user for user in file.readlines()]
    else:
        usernames_to_ignore = []
    ...
    if data["use local emotes"]:
        ...
        comments = chat.get_comments(data["json path"], emote_link_dict, local_emote_names, usernames_to_ignore)
    else: 
    ...
    layout = [
        ...
        [sg.pin(sg.Frame("",[
            [
                ...
                sg.Text("Opacity", background_color=settings["BG_COLOR"], text_color=settings["TEXT_COLOR"]),
                sg.Spin([i/100 for i in range(5,101) if not i%5], settings["ALPHA"],
                        enable_events=True, 
                        background_color=settings["BG_COLOR"],
                        text_color=settings["TEXT_COLOR"],
                        key="-OPACITY-")
            ]], key="-SETTINGS-", expand_x=True, background_color=settings["BG_COLOR"], visible=False), expand_x=True)
        ],
        [
            ...
            sg.Button("Go"),
            sg.Push(background_color=settings["BG_COLOR"]),
            sg.Button("⚙", key="-OPEN_SETTINGS-"), sg.Button("Exit")
    ...
    while True:
        ...
        if globals.key_event:
            event = "-PLAY_PAUSE-"
            _, values = window.read(timeout=250)
            globals.key_event = False
        else:
        ...
        match event:
            ...
            case "-OFF_FOCUS_EVENTS-":
                globals.off_focus_events = True if values["-OFF_FOCUS_EVENTS-"] else False
            case "-OPACITY-":
                window.set_alpha(values["-OPACITY-"])
            case "-OPEN_SETTINGS-":
